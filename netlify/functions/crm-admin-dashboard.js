    ...
    try {
        ...
        const [generalStats] = await sql`
            SELECT 
                ...
                COALESCE(SUM(s.total_cents) FILTER (WHERE sd.day >= ${dtStart} AND sd.day <= ${dtEnd} AND (${!filterSellers}::boolean OR s.seller_id = ANY(${sellerIds}::int[])) AND s.is_paid = false AND s.total_cents > 0), 0) as debt_period_cents,
                COUNT(s.id) FILTER (WHERE sd.day >= ${dtStart} AND sd.day <= ${dtEnd} AND (${!filterSellers}::boolean OR s.seller_id = ANY(${sellerIds}::int[])) AND s.is_paid = false AND s.total_cents > 0) as debt_period_count,
        ...
        const clientsQuery = await sql`
            SELECT 
                ...
                COALESCE(SUM(CASE WHEN s.is_paid = false AND s.total_cents > 0 THEN s.total_cents ELSE 0 END), 0) as total_debt
            FROM clients c
        ...
        const periodDebts = await sql`
            ...
            WHERE sd.day >= ${dtStart} AND sd.day <= ${dtEnd}
            AND (${!filterSellers}::boolean OR s.seller_id = ANY(${sellerIds}::int[]))
            AND s.is_paid = false AND s.total_cents > 0
            GROUP BY c.id, c.name, c.whatsapp
            ORDER BY period_total_cents DESC;
        `;

import { sql, recalculateAllDessertCosts } from '../netlify/functions/_db.js';
...
async function run() {
	...
	for (const item of items) {
		...
		if (found) {
			console.log(`Updating existing item: ${found.ingredient} (${item.name}) -> ${item.price}`);
			await sql`
			...
			console.log(`Creating new item: ${item.name} -> ${item.price}`);
			await sql`
	...
	console.log('Recalculating all dessert costs...');
